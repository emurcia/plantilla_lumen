create definer = administrador@`%` view indice as
select `e`.`id`                        AS `id`,
       `e`.`codigo`                    AS `codigo`,
       `e`.`dui`                       AS `dui`,
...
from ((((((((`ds_elegibilidad` `e` left join `ds_pais` `ps` on ((`e`.`id_pais_domicilio` = `ps`.`id`))) left join `ds_colonia` `cl` on ((`e`.`id_colonia_domicilio` = `cl`.`id`))) left join `ds_canton` `ct` on ((`e`.`id_canton_domicilio` = `ct`.`id`))) left join `ds_confirmados` `cf` on ((`e`.`codigo` = `cf`.`codigo`))) left join `ds_establecimiento_fisico` `et` on ((`cf`.`id_establecimiento_fisico` = `et`.`id`))) left join `ds_revision` `rv` on ((`e`.`codigo` = `rv`.`codigo`))) left join `ds_municipio` `mn` on ((`e`.`id_municipio_domicilio` = `mn`.`id`)))
         left join `ds_departamento` `mndp` on ((`mn`.`id_departamento` = `mndp`.`id`)));
